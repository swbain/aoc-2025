...
fun solution(input: List<String>, runCount: Int): Int {
    ...
    return connections.take(runCount).fold(coords.map { Circuit(setOf(it)) }) { circuits, connection ->
        ...
        val left = circuits.find { it.coords.contains(a) }
        val right = circuits.find { it.coords.contains(b) }
        ...
        circuits.toMutableList().apply {
            when {
                ...
                left != null -> {
                    removeAll { it == left || it.coords.contains(b) }
                    add(left + b)
                }
                right != null -> {
                    removeAll { it == right || it.coords.contains(a) }
                    add(right + a)
                }
            }
        }.also {
